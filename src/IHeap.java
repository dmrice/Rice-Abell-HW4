...
interface IHeap extends IBinTree {
	...
	//is the leaf bigger than the given element 
	boolean isBigger(int e);
}
class MtHeap extends MtBT implements IHeap {
	MtHeap(){}
	@Override
	//addition to MtHeap creates new DataHeap
	public IHeap addElt(int e) {
		return new DataHeap(e, new MtHeap(), new MtHeap());
	}
	@Override
	//Empty heap returns an empty heap when looking for min element
	public IHeap remMinElt() {
		return new MtHeap();
	}
	@Override
	//empty heap is always the smallest heap
	public boolean isBigger(int e) {
		return true;
	}
	@Override
	//an empty heap combined with an existing heap is the other heap
...
class DataHeap extends DataBT implements IHeap {
	...
	public LinkedList<Integer> toList() {
		...
		heapList.add(this.data);
		heapList.addAll(this.left.toList());
		heapList.addAll(this.right.toList());
