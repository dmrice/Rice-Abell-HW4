...
interface IBinTree {
	//determines whether element is within the tree
	boolean hasElt(int e);
	//return how many nodes are in the tree (num), double counts duplicates
	int size();
	//return longest branch in the tree
	int height();
...
class DataBT implements IBinTree {
	...
	public boolean correctHeap(int lastRoot) {
		if(lastRoot >= this.data) {
			return false;
		}
		else {
			lastRoot = this.data;
			return (this.left.correctHeap(lastRoot) && this.right.correctHeap(lastRoot));
		}
